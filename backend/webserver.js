...
exports.sendConfig = () => {
    let { fileTracking, ...data } = state.config
    wss.clients.forEach((ws) => {
        ws.send(
            JSON.stringify({
                type: 'newConfig',
...
exports.updateTestOverview = () => {
    let testChangesArray = []
    for (const [id, val] of Object.entries(state.testChanges)) {
        val.id = id
...
exports.updateDetailedTest = () => {
    wss.clients.forEach((ws) => {
        ws.send(
            JSON.stringify({
                type: 'detailedTestUpdate',
                data: state.detailedTestChanges,
            })
        )
    })
}
